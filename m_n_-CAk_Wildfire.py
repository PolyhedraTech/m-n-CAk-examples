...
def plot_vectorial_dots(ax, polygons, id, radius=1, type='fire', title='No title', exclusive_plot=[]):
    """
    Dibuja solo los puntos de los polígonos en un scatter plot, agrupando por color para máxima eficiencia.
    Args:
        ax (matplotlib.axes.Axes): El axis de matplotlib donde dibujar.
        polygons (list of dict): Lista de diccionarios con 'id' y 'points'.
        id (int): Identificador de la capa actual.
        ...
        exclusive_plot (list, opcional): Lista de IDs a mostrar exclusivamente.
    """
    ax.clear()
    from matplotlib.lines import Line2D
    if type=="fire":
    ...
    # Dibujar todos los puntos de cada grupo en una sola llamada scatter
    point_size = 4  # Tamaño reducido para eficiencia visual
    ...
    def nearest_neighbor_best_path(pts):
        ...
        for start in pts:
            ...
            for _ in range(1, len(pts)):
                ...
                used.add(next_pt)
            # Cerrar el ciclo (opcional, para polígonos)
...
def combination_function_on_R(point, layer, type=None):
    ...
    To simplify the example implementation we define just a a single combination_function that uses a a parameter the layer.
...
def generate_vectorial_map_from_fire_points_by_id_final(dist=1):
    ...
    for poly_id, points in FIRE_POINTS_BY_ID_FINAL.items():
        ...
        def nearest_neighbor_chain(pts, max_dist=dist):
            ...
            for _ in range(1, len(pts)):
                last = chain[-1]
                # Buscar el punto más cercano no usado y a distancia <= max_dist
                ...
                if not candidates:
                    # Si no hay candidatos, cerrar el ciclo con el más cercano
